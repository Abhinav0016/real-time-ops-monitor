...
def extract_usage_metrics(usage_records):
    """
    Calculates global usage metrics and trends.
    Assumes usage_records is sorted chronologically (from Step 2).
    - trend: 'increasing', 'decreasing', 'stable'
    - percentage_change: between last two daily records
    """
    ...
    # Simple trend based on the last two total usage snapshots
    # (In a more complex system, we'd aggregate by day first)
    # For now, let's just use the last two entries for the same region if possible, 
    # or just the last two entries total.
...
def extract_features(preprocessed_data):
    ...
    usage_metrics = extract_usage_metrics(preprocessed_data.get('usage', []))
