...
def render_tickets(issues):
    ...
    if not rows: rows = "<tr><td colspan='5' style='text-align:center;color:#475569;padding:20px'>No active tickets linked.</td></tr>"
    return f'<div class="card"><div style="display:flex; align-items:center; gap:8px; margin-bottom:16px"><span style="font-size:1.1rem">📋</span><span style="font-weight:700; font-size:rem; color:#fff">Active Jira Tickets</span></div><table class="tkt-table"><thead><tr><th>ID</th><th>Priority</th><th>Status</th><th>Assignee</th><th>Age</th></tr></thead><tbody>{rows[:6]}</tbody></table></div>'
...
if gen_btn:
    with st.spinner("Processing Operational Pipeline..."):
        ...
        metrics = extract_features(preprocess_data(raw_data))
    ...
    if result["status"] == "success":
        ...
        with l:
            ...
            st.markdown(render_tickets(result["issues"]), unsafe_allow_html=True)
